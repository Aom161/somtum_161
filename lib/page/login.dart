...
import 'package:somtum_161/page/show_product.dart';
...
class LoginScreen extends StatelessWidget {
  ...
  Future<String?> _authUser(LoginData data) async {
    ...
    try {
      final response = await http.post(
        Uri.parse('https://642021161.pungpingcoding.online/api/login'),
        body: {
          'email': data.name,
      ...
      if (response.statusCode == 200) {
        ...
        // Login Fail
        return 'ชื่อผู้ใช้หรือพาสเวิร์ดไม่ถูกต้อง กรุณาลองใหม่';
      } else {
        // Other errors
  ...
  Future<String?> _signupUser(SignupData data) async {
    ...
    try {
      final response = await http.post(
        Uri.parse('https://642021161.pungpingcoding.online/api/register'),
  ...
  Widget build(BuildContext context) {
    return FlutterLogin(
      // title: 'SOMTUM',
      theme: LoginTheme(
          primaryColor: Color.fromARGB(255, 236, 192, 96),
          ...
          pageColorDark: Color.fromARGB(255, 228, 123, 123)),
      logo: const AssetImage('assets/images/logooo.png'),
      onLogin: _authUser,
